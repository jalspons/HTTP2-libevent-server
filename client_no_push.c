...
typedef struct {
  const char *uri;              // Null-terminated URI string
  struct http_parser_url *u;    // Parsed result of the |uri|
  char *authority;              // The authority portion of the |uri|, not null-terminated
  char *path;                   // The path portion of the |uri|, not null-terminated
  size_t authoritylen;          // The length of |authority|
  size_t pathlen;               // The length of |path|
  int32_t stream_id;            // This stream id
} http2_stream_data;
...
static int on_data_chunk_recv_callback(nghttp2_session *session, uint8_t flags,
  ...
  fprintf(stderr, "[RECV %s STR]: S -------------> C\n", 
          stream_id % 2 == 0 ? "SERVER" : "CLIENT");
